...
서비스 간 결합도를 낮추고 데이터 전파가 목적인 경우 비동기 이벤트를 우선 고려합니다. 메시징 브로커로 Redpanda를 사용하며 Spring Cloud Stream과 Avro를 통해 구현합니다.

- 발행: 트랜잭셔널 아웃박스 패턴을 적용하여 DB 작업과 이벤트 발행의 원자성을 보장합니다.
- 직렬화: Apache Avro 포맷을 기본으로 사용합니다. 모든 이벤트는 Redpanda 내장 Schema Registry를 통해 스키마 버전과 호환성을 관리합니다.
- 토픽 설계: `{도메인명}`(예: `member`) 형식을 사용합니다. 상세 이벤트 타입은 Avro 메시지 본문의 type 필드에 명시합니다.
...
- 빌드 절차: 스키마 변경 시 각 서비스에서 `./gradlew generateAvroJava`를 실행하여 최신 도메인 객체를 생성해야 합니다.

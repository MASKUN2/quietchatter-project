...
### 2.4 레거시 모듈 활용 (Legacy/Archive - READ ONLY)
- `legacy-`로 시작하는 모든 서브모듈은 보관(Archive) 상태입니다.
- 모든 레거시 코드는 새로운 마이크로서비스 구현을 위한 로직 참조용으로만 사용하며, 직접적인 코드 수정, 파일 생성, 커밋을 엄격히 금지합니다.
...
서비스 간 결합도를 낮추고 데이터 전파가 목적인 경우 비동기 이벤트를 우선 고려합니다. 메시징 브로커로 Redpanda를 사용하며 Spring Cloud Stream과 JSON을 통해 구현합니다.

- 발행: 트랜잭셔널 아웃박스 패턴을 적용하여 DB 작업과 이벤트 발행의 원자성을 보장합니다.
- 직렬화: 평면화된 JSON(Flattened JSON) 포맷을 사용합니다. 인프라 복잡성을 낮추기 위해 별도의 Schema Registry는 사용하지 않습니다.
- 메시지 구조: 비즈니스 데이터와 이벤트 메타데이터의 충돌을 방지하기 위해 메타데이터 필드에는 `evt_` 접두어를 사용합니다. (예: `evt_id`, `evt_type`, `evt_time`, `evt_agg_id`)
- 토픽 설계: `{도메인명}`(예: `member`) 형식을 사용합니다. 상세 이벤트 타입은 메시지 내 `evt_type` 필드에 명시합니다.
- 독립성: 각 서비스는 자신이 소비하거나 발행하는 이벤트에 최적화된 DTO를 독립적으로 정의하여 서비스 간 결합도를 최소화합니다.

...
고객 지원: 문의 버튼을 통해 모달 형태로 문의 메시지 접수.
...
- 각 서비스는 /api/spec 엔드포인트로 최신 OpenAPI YAML 스펙 제공.
...
자세한 인프라 작업 지침은 infrastructure/README.md를 읽으십시오.
...
- 문서 표준화: 모든 서브모듈의 문서를 README.md / CLAUDE.md / GEMINI.md 구조로 단일화하고 강조 서식 및 이모티콘을 제거함.
- 프론트엔드 안정화: 린트 에러 10개를 수정하고 OnboardingContext 구조를 Fast Refresh 규칙에 맞게 개편함.
- API Gateway 보안 강화: JWT 서명 Charset 명시, SameSite=Lax 쿠키 속성 적용, AntPathMatcher 기반 정밀 경로 매칭을 구현함.
- 에러 응답 표준화: API Gateway 에러 응답을 RFC 7807 (ProblemDetail) 규격으로 전환함.
- 문서화 테스트 보완: 모든 마이크로서비스에 REST Docs 테스트를 추가/보완하여 API 명세 생성 자동화를 완료함.
- DB 마이그레이션 안전성: 모든 모듈의 Outbox 테이블 생성 스크립트에 IF NOT EXISTS 구문을 추가함.
- API 경로 동기화: API Gateway의 명세 수집 경로를 /api/spec으로 수정하고 각 서비스의 SpecController 구현과 일치시킴.
- 서브모듈 이름 변경: legacy-quiet-chatter-front-end -> frontend (현대화 및 활성화).
- 시크릿 통합: AWS Secrets Manager 9개 개별 시크릿을 단일 JSON 시크릿(quietchatter-secrets)으로 통합.
- 내부 서비스 통신 보안: X-Internal-Secret 헤더 기반 인증 적용.
